...
def correlation(**kwargs):
    ...
    axes.legend(loc='upper right', scatterpoints=1, prop={'size': 17}, fontsize=7, markerscale=7)
    axes.tick_params(axis='both', which='major', labelsize=14)
...
def percent_change(**kwargs):
    ...
    fig3 = plt.figure(num='Percent change', figsize=(18, 9), dpi=80, facecolor='w',
                      edgecolor='k')
    spec3 = fig3.add_gridspec(ncols=2, nrows=len(kwargs['stocks']))
    for h in range(len(kwargs['stocks'])):
        names.append([])
        pc.append(np.empty(len(kwargs['stocks'][h].dates)-1))
        if kwargs["labels"][h] == "high":
        ...
        axes.set_ylabel('percent change (%)', fontsize=12)
    ...
    axes2 = fig3.add_subplot(spec3[:len(kwargs['stocks'])//2, 1:])
    axes2.bar(names, prob, color=colors)
    axes2.set_ylabel('Probability of increase', fontsize=20)
    axes2.tick_params(axis='x', which='major', labelsize=18)
